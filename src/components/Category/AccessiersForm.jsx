...
const AccessiersForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchPastData = async () => {
      try {
        const response = await fetch("https://hardware-hive-backend.vercel.app/api/category/accessiers/past-data");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://hardware-hive-backend.vercel.app/api/category/accessiers", {
        method: "POST",
        body: data,
      });
      ...
      if (response.ok) {
        ...
        const pastResponse = await fetch("https://hardware-hive-backend.vercel.app/api/category/accessiers/past-data");
        const pastData = await pastResponse.json();
        if (pastResponse.ok) {
          setPastData(pastData);
        }
      } else {
        alert("Error uploading accessories: " + result.message);
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col p-4 w-1/4 space-y-3">
