...
const SparePartForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://hardware-hive-backend.vercel.app/api/category/spare-parts", {
        method: "POST",
        body: data,
      });
      ...
      if (response.ok) {
        alert("Spare part uploaded successfully!");
        setFormData({
          subcategory: "",
          brand: "",
          productName: "",
        ...
        alert("Error uploading spare part: " + result.message);
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col p-4 w-1/4 border-r space-y-3">
      <h2 className="text-xl font-bold text-center">Spare Part</h2>

      {renderDropdownWithAddOption("subcategory", "subcategory", pastData.subcategories)}
      {renderDropdownWithAddOption("brand", "add machinery name", pastData.brands)}
